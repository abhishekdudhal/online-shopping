...
if(isset($_POST['btn-login']))
{
	...
	$unpass = strip_tags($_POST['txt_npassword']);
		 $sql=mysql_query("select * from users where user_email='$umail'");
		 if($sql==false)
	{
		die(mysql_error());
	}
	...
	if($login->uppwd($uname,$unpass))
	{
		$msg="Your password has been changed succesfully : ";
 mail($umail,"PASSWOED CHANGE EB BOOK",$msg);
...
$id=$_SESSION['uname']; echo $id;?>" required />
