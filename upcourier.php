...
if(isset($_REQUEST['send']))
  ...
  $g=$_REQUEST['bp'];
	...
	if(mysql_query("UPDATE courier                                                            
SET courier_name='$b',contact='$c',owner='$d',city='$e',email_id='$f',user_name='$g' WHERE courire_id='$itemno'"))
{ 
header("location:cpanel.php?itemno=4");
}
else
{
	die(mysql_error());
...
<body>
 ...
 <li><a href="login.php"><span class="glyphicon glyphicon-user"></span> MyProfile</a></li>
 	<?php
	$sql=mysql_query("select * from courier where courire_id='$itemno'");
	if($sql==false)
	{
		die(mysql_error());
	}
	  ...
	  while($mat = mysql_fetch_array($sql)) 
        {
			    $a=isset($mat['courier_name']);
               $br=$mat['contact'];
			   $b=$mat['owner'];
			   $p=$mat['city'];
			   $pc=$mat['email_id'];
		 $aut=isset($mat['cuser_id']);		  
        ...
        <form method="post" name="f1" onSubmit="return vali()" class="form-basic" action="#">
			...
			<div class="form-row">
                <label>
                  ...
                  <input name="bn" type="text" id="bn" required="required" value="<?php echo $a;?>"onChange="return phone()">
				   </label>
            </div>
			 ...
			 <div class="form-row">
                <label>
                    <span>Contact:</span>
                 <input name="by" type="text" id="by" required="required" value="<?php echo $br;?>" onChange="return phone()">
				   </label>
            </div>
			 ...
			 <div class="form-row">
                <label>
                    <span>Owner:</span>
                 <input name="bb" type="text" id="bb" required="required" value="<?php echo $b;?>"onChange="return phone()">
                </label>
            </div>
			...
			<div class="form-row">
                <label>
                    <span>City:</span>
					<input name="bs" type="text" id="bs" required="required" value="<?php echo $p;?>" onChange="return phone()">
					</label>
            </div>
             ...
             <div class="form-row">
                <label>
                    <span>Email-Id:</span>
                   <input name="ba" type="text" id="ba" required="required" value="<?php echo $pc;?>" onChange="return phone()">
					</label>
            </div>
