...
if(isset($_REQUEST['send']))
  ...
  $f=$_REQUEST['bm'];
	...
	if(mysql_query("UPDATE contactus                                                              
SET srno='$a',name='$b',emailid='$c',mobileno='$d',purpose='$e',discription='$f'
WHERE srno='$itemno'"))
{ 
header("location:cpanel.php?itemno=9");
}
else
{
	die(mysql_error());
...
<?php
	$sql=mysql_query("select * from contactus where srno='$itemno'");
	if($sql==false)
	{
		die(mysql_error());
	}
...
<input name="by" type="text" id="by" required="required" value="<?php echo $c;?>" onChange="return phone()">
    ...
    <input name="bl" type="text" id="bl" required="required" value="<?php echo $e;?>"onChange="return phone()">
     ...
     <input name="bm" type="text" id="bm" required="required" value="<?php echo $f;?>" onChange="return phone()">
				 </label>
            </div>
